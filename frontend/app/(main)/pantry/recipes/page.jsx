  ...
  return (
    <div className='min-h-screen bg-stone-50 pt-24 pb-16 px-4'>
      <div className='container mx-auto max-w-6xl'>
        ...
        {/* Error State */}
        {error && (
          <div className='bg-red-50 p-12 text-center border-2 border-red-200'>
        ...
        {/* Rate Limit Reached - Only if strictly denied due to limit (we'd need a flag, but relying on data check is weak.
            However, assuming error handles the actual failures, undefined data with no error means it just hasn't loaded or initial state.
            We should only show this if we are sure it was a rate limit, usually passed via data or error.
            Actually, arcjet throws an error for rate limit. The action catches it and throws a new Error.
            So rate limit effectively shows up as an error!
            The previous logic `!loading && recipesData === undefined` catches the initial state too.
         */}

        {/* Initial Loading / Empty State Prevention */}
        {!loading &&
          !error &&
          recipesData === undefined &&
          // detailed checking to avoid flash
          null}

        {/* If code reached here implies success=false handled? No.
            Data might be { success: false, message: ... }
          */}
      </div>
    </div>
  )
