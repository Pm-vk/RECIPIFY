...
export async function getOrGenerateRecipe (formData) {
  try {
    ...
    // Wrap database calls intry/catch to non-blocking
    try {
      ...
      console.error(
        'Database search failed, proceeding to generation:',
        dbError
      )
    ...
    // Use flash-latest which should be the most stable flash model
...
`
    ...
    try {
      ...
      while (attempts < maxAttempts) {
        try {
          ...
          if (err.message.includes('429') && attempts < maxAttempts - 1) {
            console.log(
              `⚠️ Rate limit hit, retrying in ${(attempts + 1) * 2}s...`
            )
            await new Promise(resolve =>
              setTimeout(resolve, (attempts + 1) * 2000)
            )
            attempts++
          } else {
            throw err
    ...
    // Wrap in try-catch to ensure we return the recipe even if saving fails
    let recipeId = null
    try {
      ...
      if (!createRecipeResponse.ok) {
        ...
        // We don't throw here, we just return the generated recipe without ID
      } else {
      ...
      // Continue to return recipe to user
    }
    ...
    return {
      ...
      recipeId: recipeId, // Might be null if save failed
      isSaved: false,
      fromDatabase: false,
    ...
    // Return a safe error structure that won't crash the frontend
    return {
      success: false,
